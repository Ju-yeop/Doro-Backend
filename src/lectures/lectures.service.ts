...
export class LectureService {
    ...
    async createEdu({
        ...
        budget,
        overall_remark,
        detail_classes}: CreateEduInput): Promise<CreateEduOutput>{
        try {
            const details: Detail_class_info[] = [];
            const client = await this.client.findOne({ where: { name, phone_number } })
            if (!client) {  // client가 없을 경우
                ...
                await createAndSave_Class(newClient, details, this.client, this.overall_class_info, this.detail_class_info);
                return {
                    ok: true
                }
            };
            // client가 이미 있을 경우
            await createAndSave_Class(client, details, this.client, this.overall_class_info, this.detail_class_info);
            return {
                ok: true
            }
        } catch (error) {
